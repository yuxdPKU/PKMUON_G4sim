...
void fit_smear(){
        ...
        //TCut cut = "fabs(dca_x_smear)<500 && fabs(dca_y_smear)<500 && fabs(dca_z_smear)<500";
        TCut cut = "fabs(dca_x_smear)<500 && fabs(dca_y_smear)<500 && fabs(dca_z_smear)<1000 && angle>0.5";
        //TCut cut = "fabs(dca_x_smear)<500 && fabs(dca_y_smear)<500 && fabs(dca_z_smear)<500 && angle<0.5";
        TTree *intree2 = intree->CopyTree(cut);

        double xmin=-1000, xmax=1000, xbins=50;
	RooRealVar *dca_z_smear=new RooRealVar("dca_z_smear","dca_z_smear",xmin,xmax) ;
        //dca_z_smear->setRange("fitRange", -800, 800);
        dca_z_smear->setRange("fitRange", -400, 400);
        RooDataSet *data = new RooDataSet("data","data",intree2,RooArgList(*dca_z_smear));

	TString a("Events/"); char b[20];  sprintf(b, "(%.1f",(xmax-xmin)/xbins); TString d(" mm)");
	TString ytitle = a + b + d;

	// Create two Gaussian PDFs g1(x,mean1,sigma) anf g2(x,mean2,sigma) and their parameters
	RooRealVar *mean1=new RooRealVar("mean1","mean of gauss1", -1.8533e+01, -200, 200);
	RooRealVar *mean2=new RooRealVar("mean2","mean of gauss2", -1.0000e+01, -200, 200);
	RooRealVar *sig1frac=new RooRealVar("sig1frac", "fraction", 2.0565e-01, 0, 1.0);
	RooRealVar *sigma1=new RooRealVar("sigma1","width of gauss1", 2.0000e+01, 0, 500);
	RooRealVar *sigma2=new RooRealVar("sigma2","width of gauss2", 3.0000e+01, 0, 500);

	RooGaussian *gaus1=new RooGaussian("gaus1","Signal component 1",*dca_z_smear,*mean1,*sigma1) ;
	RooGaussian *gaus2=new RooGaussian("gaus2","Signal component 2",*dca_z_smear,*mean2,*sigma2) ;
	RooAddModel  *sig=new RooAddModel("sig","",RooArgList(*gaus1,*gaus2),*sig1frac) ;//pdf!!

//      RooBifurGauss *gaus3=new RooBifurGauss("gaus3","BifurGauss",*dca_z_smear,*mean1,*sigma1,*sigma2);
...
//	RooAddPdf  *model=new RooAddPdf("model","",RooArgList(*gaus3),RooArgList(nsig)) ;//create pdf
	...
	m_fitres->Print("v");
		...
		// --- Draw ---
		RooPlot* frame = dca_z_smear->frame(xmin,xmax,xbins);
        ...
        pt->Draw();
        Canvas->Update();
        Canvas->Print("./figure/fit_smear.eps");
